...
function initDashboard() {
    ...
    async function activeSOS(mode) {
    console.log("SOS activated! Sending location...");
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                const response = await fetch("http://localhost:5000/sendSOS", {
                    method: "POST",
...
//     // SOS Active Modal
//     const sosActiveModal = document.getElementById('sos-active-modal');
//     const stopSosBtn = document.getElementById('stop-sos-btn');
//     const updateLocationBtn = document.getElementById('update-location-btn');
    
//     if (stopSosBtn) {
//         stopSosBtn.addEventListener('click', function() {
...
//             alert('Location updated and sent to emergency contacts.');
//         });
//     }
// }

// // Activate SOS
// function activateSOS(mode) {
//     // Show SOS active modal
//     const sosActiveModal = document.getElementById('sos-active-modal');
//     if (sosActiveModal) {
//         openModal(sosActiveModal);
...
//         setTimeout(() => {
//             alert(`SOS alert activated in ${mode} mode. Your location has been sent to your emergency contacts.`);
//         }, 1000);
//     }
// }
...
//     // Hide SOS active modal
//     const sosActiveModal = document.getElementById('sos-active-modal');
//     if (sosActiveModal) {
//         closeModal(sosActiveModal);
        
//         // Stop timer
//         stopSOSTimer();
        
//         // Show confirmation
//         alert('SOS alert has been deactivated.');
//     }
// }
...
async function startRecording() {
    if (recordingActive) return; // Prevent multiple recordings
    ...
    try {
        ...
        mediaRecorder.onstop = async () => {
            ...
            recordedChunks = [];
            const url = await uploadToSupabase(blob); // Upload to Supabase
            console.log("Stored video at:", url);
        };
...
// Initialize Supabase
const supabaseUrl = 'https://qfkurcgfivvhroudxvjr.supabase.co';  // Replace with your Supabase URL
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFma3VyY2dmaXZ2aHJvdWR4dmpyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDM0ODIxNjYsImV4cCI6MjA1OTA1ODE2Nn0.6a7HA6kXkl5oEGwGAJedr-MuJHDmH7bO7sB6msiwG5E';  // Replace with your Anon key
const supabase = createClient(supabaseUrl, supabaseKey);

async function uploadToSupabase(blob) {
    const fileName = `recording-${Date.now()}.webm`; // Unique file name

    // Upload to Supabase Storage
    const { data, error } = await supabase.storage
        .from('sosvideos') // Your bucket name
...
async function getSignedUrl(fileName) {
    try {
        ...
        const { data, error } = await supabase
            .storage
            .from("sosvideos")
            .createSignedUrl(fileName, 600); // 600 sec = 10 minutes
...
// Activate SOS (Start Recording)
// function activateSOS() {
//     startRecording();
// }

// // Deactivate SOS (Stop Recording)
// function deactivateSOS() {
//     stopRecording();
// }
...
document.getElementById('add-contact-btn').addEventListener('click', function() {
    ...
    const newContactHTML = `
        <div class="emergency-contact">
            ...
            <button class="delete-contact-btn" onclick="deleteContact(${contactCount})">
                <i class="fas fa-trash"></i>
            </button>
        </div>
    `;
...
if (highAlertBtn) {
    highAlertBtn.addEventListener("click", async function () {
      ...
      try {
        const alertRes = await fetch("http://localhost:5000/api/sent-alert", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      // ✅ Stop recording if the user closes or refreshes the tab
